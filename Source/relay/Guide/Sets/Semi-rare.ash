...
void SemirareGenerateDescription(string [int] description)
{
    ...
    int permanent_pool_skill = floor(2.0 * sqrt(get_property_int("poolSharkCount")));
    if (permanent_pool_skill < 10 && to_item("7301").available_amount() > 0) {
        ...
        semirares.listAppend(LocationChoiceMake($location[The Haunted Billiards Room], "|*Train permanent pool skill (need " + visits_for_next_level + " more visits for level " + (permanent_pool_skill + 1) + ")", 0));
    }
